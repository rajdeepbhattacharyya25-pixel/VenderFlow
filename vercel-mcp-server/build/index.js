...
const vercel_client_js_1 = require("./vercel-client.js");
...
const vercel = new vercel_client_js_1.VercelClient(API_TOKEN);
...
function formatError(error) {
    if (axios_1.default.isAxiosError(error)) {
        return `Vercel API Error: ${error.response?.status} - ${JSON.stringify(error.response?.data)}`;
    }
    ...
    try {
        ...
        switch (name) {
            ...
            case "vercel_add_domain": {
                const { projectId, name: domainName } = args;
            ...
            case "vercel_remove_domain": {
                const { projectId, domainName } = args;
            ...
            case "vercel_create_env_var": {
                const { projectId, key, value, target } = args;
