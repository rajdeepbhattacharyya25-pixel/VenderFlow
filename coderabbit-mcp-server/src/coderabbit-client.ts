...
export interface ReportParams {
    ...
    prompt?: string;       // Custom prompt for report
    promptTemplate?: string; // Pre-defined template (e.g., "Daily Standup Report", "Sprint Report")
    groupBy?: string;      // Primary grouping
    parameters?: Record<string, any>; // Additional filters
}
...
export class CodeRabbitClient {
     ...
     */
    analyzeDependencies(packageJson: string): AnalysisResult[] {
        const issues: AnalysisResult[] = [];
     ...
     */
    generateHealthSummary(issues: AnalysisResult[]): SystemHealthSummary {
     ...
     */
    formatAnalysisResults(issues: AnalysisResult[]): string {
        if (issues.length === 0) {
