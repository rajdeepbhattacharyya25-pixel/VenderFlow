...
const ScrollStack: React.FC<ScrollStackProps> = ({
    ...
    const getElementOffset = useCallback(
        (element: HTMLElement, index: number = -1) => {
            if (useWindowScroll) {
                let rectTop = element.getBoundingClientRect().top;
                if (index >= 0) {
                    const lastT = lastTransformsRef.current.get(index);
                    if (lastT) {
                        rectTop -= lastT.translateY;
                    }
                }
                return rectTop + window.scrollY;
            } else {
                return element.offsetTop;
            }
        },
        [useWindowScroll, lastTransformsRef]
    );
    ...
    const updateCardTransforms = useCallback(() => {
        ...
        const endElementTop = endElement ? getElementOffset(endElement as HTMLElement, -1) : 0;
