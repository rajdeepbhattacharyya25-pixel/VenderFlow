    ...
    useEffect(() => {
        ...
        class MouseClass {
            ...
            update() {
                ...
                if (this.isAutoActive && !this.takeoverActive) {
                    // Internal boost for auto-demo to ensure clear trails even with low prop intensity
                    this.diff.multiplyScalar(this.autoIntensity * 1.5);
                } else if (!this.isAutoActive || this.takeoverActive) {
                    // Less aggressive dampening to ensure visibility
                    this.diff.multiplyScalar(0.7);
                }
            }
        }
        ...
        class AutoDriver {
            ...
            pickNewTarget() {
                ...
                const startX = side === 1 ? 1 : side === 3 ? -1 : (r() * 2 - 1) * (1 - this.margin);
                const startY = side === 0 ? 1 : side === 2 ? -1 : (r() * 2 - 1) * (1 - this.margin);

                const endX = side === 1 ? -1 : side === 3 ? 1 : (r() * 2 - 1) * (1 - this.margin);
                const endY = side === 0 ? -1 : side === 2 ? 1 : (r() * 2 - 1) * (1 - this.margin);

                this.start.set(startX, startY);
                this.target.set(endX, endY);
                this.current.copy(this.start);

                // Sync mouse to avoid velocity jumps
                this.mouse.coords.copy(this.start);
                this.mouse.coords_old.copy(this.start);
            ...
            update() {
                ...
                // Relaxed: Only stop if hover is inside or takeover is active
                ...
                const elapsed = now - this.pathStartTime;
                ...
                // If movement is done but we are in the "waiting" phase of the 5s cycle
                if (elapsed > this.pathDuration) {
                    // Move mouse off-screen to allow dissipation
                    this.mouse.coords.set(-10, -10);
                    this.mouse.coords_old.set(-10, -10);
                    return;
                }
                const t = Math.min(1, elapsed / this.pathDuration);
                let ramp = 1;
                ...
                // Add organic sine-wave offset
                const dx = this.target.x - this.start.x;
                const dy = this.target.y - this.start.y;
                const perpX = -dy;
                const perpY = dx;
                const offset = Math.sin(t * Math.PI) * 0.15 * Math.sin(now * 0.005);

                this.current.lerpVectors(this.start, this.target, t);
                this.current.x += perpX * offset;
                this.current.y += perpY * offset;
...
`;
        ...
        class ExternalForce extends ShaderPass {
            ...
            constructor(simProps: any) {
                ...
                super.init(); // avoid calling this.init(simProps) directly with super.init() being called inside
