...
interface OrderItem {
    ...
    image: string;
    size: string;
    id: string; // Changed from number to string to match product UUIDs
}
...
export const Orders: React.FC<OrdersProps> = ({ onNavigate, onAddToCart, showToast, sellerId, storeCustomer }) => {
    ...
    React.useEffect(() => {
        const fetchOrders = async () => {
            ...
            // Get current authenticated user ID to ensure matches RLS and Insert logic
            const { data: { user } } = await supabase.auth.getUser();

            if (user) {
                query = query.eq('customer_id', user.id);
            } else if (storeCustomer.customer_id) {
                // Fallback for legacy/loose modes (unlikely to work with strict RLS but kept for safety)
                query = query.eq('customer_id', storeCustomer.customer_id);
            } else if (storeCustomer.id) {
                query = query.eq('customer_id', storeCustomer.id);
            }
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2 sm:pb-0 hide-scroll">
                        {['All', 'pending', 'processing', 'shipped', 'delivered', 'cancelled'].map(filter => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-all border ${orderFilter === filter
