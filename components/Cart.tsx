...
export const Cart: React.FC<CartProps> = ({ 
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 pt-8 pb-32 animate-in slide-in-from-bottom-4 duration-500 min-h-screen">
              ...
              >
                ...
                <div className="flex-grow flex flex-col justify-between py-0.5">
                  ...
                  <div className="flex flex-wrap items-end justify-between gap-4 mt-3">
                    ...
                    {/* Quantity */}
                    <div className="flex items-center bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg order-1 md:order-2 ml-auto md:ml-0">
                      <button 
                        onClick={() => onUpdateQuantity(item.product.id, item.size, -1)}
                        className="w-8 h-8 flex items-center justify-center text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors rounded-l-lg disabled:opacity-30"
                      ...
                      <button 
                        onClick={() => onUpdateQuantity(item.product.id, item.size, 1)}
                        className="w-8 h-8 flex items-center justify-center text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors rounded-r-lg disabled:opacity-30 disabled:cursor-not-allowed"
                        disabled={isLowStock} // Simulate disable at limit
                      >
                        +
                      </button>
                    </div>
      ...
      <div className="lg:hidden fixed bottom-[57px] left-0 right-0 bg-white dark:bg-surface-dark border-t border-gray-100 dark:border-gray-800 p-4 shadow-xl z-30 pb- safe-area-inset-bottom">
         <div className="flex items-center gap-4">
            <div className="flex-1">
               <p className="text-xs text-gray-500 dark:text-gray-400">Total to Pay</p>
               <p className="text-xl font-bold text-gray-900 dark:text-white">₹{total.toFixed(2)}</p>
            </div>
            <button 
              onClick={() => onCheckout(selectedItems)}
              disabled={selectedCount === 0}
              className="flex-1 bg-primary text-white font-bold py-3.5 px-6 rounded-xl shadow-md disabled:opacity-70 flex items-center justify-center gap-2"
            >
               Checkout ({selectedCount})
            </button>
         </div>
