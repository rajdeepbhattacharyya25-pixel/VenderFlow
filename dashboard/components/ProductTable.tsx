    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 mt-6 overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[600px] border-collapse">
          <thead>
            <tr className="text-left border-b border-muted/10">
              <th className="pb-4 text-xs font-semibold text-muted uppercase tracking-wider pl-4">Product</th>
              <th className="pb-4 text-xs font-semibold text-muted uppercase tracking-wider">Price</th>
              <th className="pb-4 text-xs font-semibold text-muted uppercase tracking-wider">Orders</th>
              <th className="pb-4 text-xs font-semibold text-muted uppercase tracking-wider">Stock</th>
              <th className="pb-4 text-xs font-semibold text-muted uppercase tracking-wider pr-4 text-right">Amount</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-muted/5">
            {products.map((product) => (
              <tr key={product.id} className="group card-hover hover:bg-gray-50 dark:hover:bg-white/5 transition-colors rounded-lg relative">
