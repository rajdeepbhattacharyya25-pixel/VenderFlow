...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onQuickView, isWishlisted, onToggleWishlist, onAddToCart, compact = false }) => {
  ...
  return (
    <div className="group flex flex-col h-full relative">
      {/* Image Container */}
      <div className={`relative bg-[#F9F9F9] dark:bg-gray-200 rounded-2xl overflow-hidden ${compact ? 'mb-2' : 'mb-4'} aspect-[3/4] shadow-sm group-hover:shadow-md transition-all duration-300 border border-transparent group-hover:border-gray-100 dark:group-hover:border-gray-700`}>
        ...
        <div className={`absolute ${compact ? 'top-2 right-2' : 'top-3 right-3'} z-20 flex flex-col gap-2 translate-x-10 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300`}>
          <button 
            ...
            className={`rounded-full shadow-sm hover:shadow-md transition-all transform active:scale-90 ${compact ? 'p-1.5' : 'p-2.5'} ${
              isWishlisted 
        ...
        {/* Product Image */}
        <div className={`w-full h-full ${compact ? 'p-3' : 'p-6'} cursor-pointer flex items-center justify-center overflow-hidden`} onClick={() => onQuickView(product)}>
           <img 
            src={product.image} 
            alt={product.name} 
